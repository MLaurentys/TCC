...
Domineering is played by placing, or marking, a 2x1 rectangle on the board, or drawing. Left plays in the horizontal and right in the vertical. The reader is invited to play the position above a few times, and realize that the player starting it is always able to win. In fact, with perfect play, both players can win with a move to spare. The quest for calculating the advantage a player has, so far, can be been summarized by the question: ``Who is ahead, by how much?". However, in positions such as the one above, a new question is more important: ``How big is the next move". \todo{Berlekamp 1}.
...
Switches are the most basic non-numbers. A switch is a non-numbers $G$ that both left's and right's best moves are numbers, but $G^L >= G^R$. It is extremely easy to find the \defi{temperature} and \defi{bias} of such games. 
